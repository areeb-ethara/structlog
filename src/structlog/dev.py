...
from .exceptions import MultipleConsoleRenderersConfiguredError, NoConsoleRendererConfiguredError
...
class ColumnStyles:
    ...
    kv_key: str
    kv_value: str
_colorful_styles = ColumnStyles(reset=RESET_ALL, bright=BRIGHT, level_critical=RED, level_exception=RED, level_error=RED, level_warn=YELLOW, level_info=GREEN, level_debug=GREEN, level_notset=RED_BACK, timestamp=DIM, logger_name=BLUE, kv_key=CYAN, kv_value=MAGENTA)
_plain_styles = ColumnStyles(reset='', bright='', level_critical='', level_exception='', level_error='', level_warn='', level_info='', level_debug='', level_notset='', timestamp='', logger_name='', kv_key='', kv_value='')
...
class LogLevelColumnFormatter:
    ...
    def __init__(self, level_styles: dict[str, str], reset_style: str, width: int | None=None) -> None:
        ...
        if level_styles:
            self.width = 0 if width == 0 else len(max(self.level_styles.keys(), key=len))
...
class ConsoleRenderer:
    ...
    *after* the log line. If Rich_ or better-exceptions_ are present, in colors
    ...
    def __init__(self, pad_event_to: int=_EVENT_WIDTH, colors: bool=_has_colors, force_colors: bool=False, repr_native_str: bool=False, level_styles: dict[str, str] | None=None, exception_formatter: ExceptionRenderer=default_exception_formatter, sort_keys: bool=True, event_key: str='event', timestamp_key: str='timestamp', columns: list[Column] | None=None, pad_level: bool=True, pad_event: int | None=None):
        if pad_event is not None:
            if pad_event_to != _EVENT_WIDTH:
                raise ValueError('Cannot set both `pad_event` and `pad_event_to`.')
            warnings.warn('The `pad_event` argument is deprecated. Use `pad_event_to` instead.', DeprecationWarning, stacklevel=2)
        ...
        self._level_styles = self.get_default_level_styles(colors) if level_styles is None else level_styles
        ...
        def add_meaningless_arg(arg: str) -> None:
            to_warn.append(f'The `{arg}` argument is ignored when passing `columns`.')
        if pad_event_to != _EVENT_WIDTH:
    ...
    @classmethod
    def get_default_column_styles(cls, colors: bool, force_colors: bool=False) -> ColumnStyles:
        """
    ...
    def __call__(self, logger: WrappedLogger, name: str, event_dict: EventDict) -> str:
        ...
        kvs = [col.formatter(col.key, val) for col in self.columns if (val := event_dict.pop(col.key, _NOTHING)) is not _NOTHING] + [self._default_column_formatter(key, event_dict[key]) for key in (sorted(event_dict) if self._sort_keys else event_dict)]
        sio = StringIO()
